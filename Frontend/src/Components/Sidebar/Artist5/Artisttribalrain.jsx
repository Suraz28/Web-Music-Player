...
import "./Artisttribalrain.css";


const Artisttribalrain = () => {
    const { setSelectedSong } = useMusicContext();
    const [audioDurations, setAudioDurations] = useState({});

    const fetchArtists = async () => {
        const response = await fetch("http://localhost:3000/TribalRain");
        if (!response.ok) {
    ...
    return (
        <div className='sidegenres'>
            <div className='sidegenresdiv'>
                ...
                <img src='./public/Artists/Images/Tribal Rain.jpg' style={{ width: '100%', height: '30vh', objectFit: 'cover', borderRadius: "10px" }}/>
                <div className="genreList">
                    {artists.map((artist, id) => (
                        <div className='genresitemlists' key={id} onClick={() => handleSongSelect(artist)}>
...
export default Artisttribalrain;
