...
const Artistsujan = () => {
    ...
    const fetchArtists = async () => {
        const response = await fetch("http://localhost:3000/SujanChapagain");
        if (!response.ok) {
        ...
        <div className='sidegenres'>
            <div className='sidegenresdiv'>
                <div className='sidegenredivtexts'>
                   <h3>Sujan Chapagain</h3>
                   <Link to="/artists" style={{textDecoration: "none", color: "inherit", cursor: "pointer"}}>
                   <RxCross2 style={{fontSize: "25px", cursor: "pointer"}}/>
                   </Link>
                </div>
                <img src='./public/Artists/Images/Sujan Chapagain.jpg' style={{ width: '100%', height: '30vh', objectFit: 'cover', borderRadius: "10px" }}/>
